...
class WP_REST_Post_Statuses_Controller extends WP_REST_Controller {
	...
	public function prepare_item_for_response( $status, $request ) {
		...
		if ( 'publish' === $status->name ) {
			$response->add_link( 'archives', rest_url( sprintf( '/%s/posts', $this->namespace ) ) );
		} else {
			$response->add_link( 'archives', add_query_arg( 'status', $status->name, rest_url( sprintf( '/%s/posts', $this->namespace ) ) ) );
		}
