...
class WP_JSON_Comments_Controller extends WP_JSON_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( isset( $request['post'] ) ) {
			...
			if ( $post && ! $this->check_read_post_permission( $post ) ) {
				return new WP_Error( 'json_cannot_read_post', __( 'Sorry, you cannot read the post for this comment.' ) );
			}
		}

		if ( ! empty( $request['context'] ) && 'edit' == $request['context'] && ! current_user_can( 'manage_comments' ) ) {
			return new WP_Error( 'json_forbidden_context', __( 'Sorry, you cannot view comments with edit context.' ), array( 'status' => 403 ) );
		}
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! $this->check_read_permission( $comment ) ) {
			return new WP_Error( 'json_cannot_read', __( 'Sorry, you cannot read this comment.' ), array( 'status' => 403 ) );
		}

		$post = get_post( $comment->comment_post_ID );

		if ( $post && ! $this->check_read_post_permission( $post ) ) {
			return new WP_Error( 'json_cannot_read_post', __( 'Sorry, you cannot read the post for this comment.' ), array( 'status' => 403 )  );
		}

		if ( ! empty( $request['context'] ) && 'edit' === $request['context'] && ! current_user_can( 'moderate_comments' ) ) {
			return new WP_Error( 'json_forbidden_context', __( 'Sorry, you cannot view this comment with edit context.' ), array( 'status' => 403 ) );
		}
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( $post ) {
			...
			if ( ! comments_open( $post->ID ) ) {
				return new WP_Error( 'json_comment_closed', __( 'Sorry, comments are closed on this post.' ), array( 'status' => 403 ) );
			}
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( $comment && ! $this->check_edit_permission( $comment ) ) {
			return new WP_Error( 'json_cannot_edit', __( 'Sorry, you can not edit this comment.' ), array( 'status' => 403 ) );
		}
